...
xlnet = TFXLNetModel.from_pretrained(model_name, from_pt=True)
xlnet.transformer._layers[0].trainable = False
for x in xlnet.transformer._layers[1][:6]: x.trainable = False

bert_inputs = tf.keras.layers.Input(shape=(max_seq_len,), dtype=tf.int32)
seq_output = xlnet(bert_inputs)[0]
