...
def rule_check(con_ls):
    ...
    for i in range(l):
        sin_con = con_ls[i].split
        if 'is' in sin_con or 'in' in sin_con or '之一' in con_ls[i] or '兼' in con_ls[i] or '及' in con_ls[i] or '和' in con_ls[i] or con_ls[i][0] == '类' or '为' in con_ls[i] or '称' in con_ls[i] or '等' in con_ls[i] or '是' in con_ls[i] or '于' in con_ls[i] or '在' in con_ls[i] or '以' in con_ls[i] or '与' in con_ls[i]:
            if con_ls[i] in r:
                ind = r.index(con_ls[i])
                r.pop(ind)
...
if __name__ == '__main__':
    ...
    with open('Bert_mrc_rule_result_probase.txt', 'w', encoding="utf-8") as f1:
        for info in dataset.keys():
