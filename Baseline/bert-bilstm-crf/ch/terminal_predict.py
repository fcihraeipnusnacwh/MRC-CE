...
def predict_online(text):
    ...
    with graph.as_default():
        ...
        pred_label_result = convert_id_to_label(pred_ids_result, id2label)[0]
        l = len(pred_label_result)
        i = 0
        con = []
        flag = 0
        s = ''
...
if __name__ == "__main__":
    ...
    for key in testset.keys():
        pred_con = ','.join(list(set(predict_online(key))))
        if pred_con != testset[key]:
            print(pred_con)
            print(testset[key])
            wrong.append([key, testset[key], pred_con])
    '''
    for key in testset.keys():
        pred_con = ','.join(list(set(predict_online(key))))
        wrong.append([key, testset[key], pred_con])
            
    with open('BERT_BiLSTM_CRF_result.txt', 'w', encoding="utf-8") as f1:
        for ele in wrong:
            f1.write(ele[0])
            f1.write('\t')
            f1.write(ele[1])
